import { Helmet } from 'react-helmet-async';
import { SuburbHero } from '../../components/suburb/suburb-hero';
import { SuburbServices } from '../../components/suburb/suburb-services';
import { SuburbWhyChoose } from '../../components/suburb/suburb-why-choose';
import { SuburbProcess } from '../../components/suburb/suburb-process';
import { SuburbFAQ } from '../../components/suburb/suburb-faq';
import { BeforeAfterGallery } from '../../components/before-after-gallery';
import { GoogleReviews } from '../../components/google-reviews';
import { QuoteForm } from '../../components/quote-form';
import { StickyCallButton } from '../../components/sticky-call-button';
import { generateSuburbFAQSchema } from '../../utils/suburb-faq-schema';

export default function Currambine() {
  const faqSchema = generateSuburbFAQSchema('Currambine');
  
  return (
    <>
      <StickyCallButton />
      <Helmet>
        <title>Pressure Cleaning Currambine WA | House & Driveway Washing | Premier Pressure Solutions WA</title>
        <meta name="description" content="Professional pressure cleaning services in Currambine. Specialising in house washing, driveway cleaning, roof cleaning & more. Serving Currambine homes. Call 0452 579 657!" />
        <meta name="keywords" content="pressure cleaning Currambine, house washing Currambine, driveway cleaning Currambine, roof cleaning Currambine, Currambine pressure washing" />
        <link rel="canonical" href="https://premierpressuresolutionswa.com.au/suburbs/currambine" />
        
        {/* FAQ Schema */}
        <script type="application/ld+json">
          {JSON.stringify(faqSchema)}
        </script>
      </Helmet>

      <SuburbHero
        suburbName="Currambine"
        tagline="Pressure Cleaning Currambine - Family Home Specialists"
        description="Expert pressure cleaning for Currambine's modern family homes. Keep your property looking pristine with professional cleaning services tailored to northern suburbs living."
      />

      <SuburbServices suburbName="Currambine" />
      <BeforeAfterGallery />
      <SuburbWhyChoose suburbName="Currambine" />

      <section className="py-20 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-bold text-[#0a1628] mb-8 text-center">
              Get Your Free Quote Today
            </h2>
            <QuoteForm />
          </div>
        </div>
      </section>

      <SuburbProcess suburbName="Currambine" />

      <SuburbFAQ suburbName="Currambine" />

      <GoogleReviews />
    </>
  );
}